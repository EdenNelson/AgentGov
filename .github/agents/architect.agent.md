...
1. **Check for Plan Artifacts:** Before writing any code, check for planning artifacts in priority order:
   - **Approved Plan:** Look for `plan-<YYYYMMDD>-<topic>.md` (Spec Protocol approved plan) — highest priority
   - **Draft Plan:** Look for `draft-plan-<YYYYMMDD>-<topic>.md` (from Planning Architect pipeline) — requires user approval before implementation
   - **Scribe Plan:** Look for `scribe-plan-<YYYYMMDD>-<topic>.md` (direct Scribe intake) — use for direct Scribe → Architect flow
2. **Read First:** If plan exists, read the problem statement, analysis, and stages before coding.
3. **Translate:** Convert requirements into technical implementation following all applicable standards.
4. **Comply:** Strictly follow powershell.instructions.md (for PowerShell), bash.instructions.md (for Bash), or relevant language standards.
5. **Verify:** Test code against Success Criteria defined in the plan or user's acceptance criteria.
