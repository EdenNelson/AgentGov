...
1. **STANDARDS_CORE.md §1.1** (Universal Principle):
...
2. **STANDARDS_POWERSHELL.md §File Encoding** (Platform-Specific Mandate):
...
**Deeper Problem:** The STANDARDS_POWERSHELL.md rule implies that agents on macOS/Linux **must convert** files to CRLF. But this creates a side effect (encoding change that must be undone or preserved as-is). True idempotency would mean: "Run once, run again, same result." CRLF mandate breaks this.
...
- Contradicts STANDARDS_CORE.md's universal principle
...
**STANDARDS_POWERSHELL.md Rationale:**
...
- From PERSONA.md: "We treat all endpoints (Windows, macOS, Linux) as a single logical fleet"
...
- **Con:** Contradicts STANDARDS_CORE.md principle
...
- Create STANDARDS_POWERSHELL_GPO.md for Group Policy scripts (CRLF required)
- Create STANDARDS_POWERSHELL_CORE.md for cross-platform scripts (LF preferred)
...
1. **STANDARDS_CORE.md idempotency is non-negotiable** — it is a universal principle
...
### STAGE 2: Update STANDARDS_POWERSHELL.md — Conditional CRLF Rule

**Objective:** Replace blanket mandate with conditional rule

**Deliverables:**
- [ ] STANDARDS_POWERSHELL.md §File Encoding, rewritten:
...
### STAGE 3: Update STANDARDS_CORE.md — Clarify Idempotency Definition

**Objective:** Explicitly state that idempotency includes file encoding

**Deliverables:**
- [ ] STANDARDS_CORE.md §1.1 Idempotency, clarified:
  ...
  - Link to updated STANDARDS_POWERSHELL.md conditional CRLF rule

**Checkpoint:** STANDARDS_CORE.md updated
...
1. Do you agree that idempotency (STANDARDS_CORE.md) should override platform-specific file encoding requirements?
...
- [STANDARDS_CORE.md](../../STANDARDS_CORE.md) §1.1 (Idempotency principle)
- [STANDARDS_POWERSHELL.md](../../STANDARDS_POWERSHELL.md) §File Encoding (current CRLF mandate)
- [PERSONA.md](../../PERSONA.md) §Institutional Memory (cross-platform fleet)
